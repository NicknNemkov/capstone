...
areas in the US (including US territories). Each row represents an occupation title, its location in the US, and various wage statistic values (total employment, mean wage estimates, etc.).
...
- Is there a noticable trend between a person’s salary and their sex, what about their race?
...
I trained a logistic regression model to try to predict an employee's salary category (greater than $50,000 or  less than or equal to $50,000) based on various personal characteristics (age, sex, marital status, education level, etc.).
```{python}
...
The map (@fig-choropleth_hrly_wage) shows that states like New York and California have an average hourly pay of around $37. On the low end, the state of Mississippi has a mean hourly wage of only $22.87. The majority of the states have an hourly salary of slightly below $30.
