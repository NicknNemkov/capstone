...
The _state_M2023_dl.xlsx_ dataset contains the 2023 occupational employment and wage estimates, calculated with
data collected from employers in all industry sectors in metropolitan and nonmetropolitan
areas in the US (including US territories).
...
This U.S. Cansus Bureau dataset collects the estimated Components of Resident Population
Change, and Rates of the Components of Resident Population Change for the United States,
States, District of Columbia, and Puerto Rico from 2020 to 2023.

:::{.callout-important}
Only the _POPESTIMATE2023_ column will be used from NST-EST2023-ALLDATA.csv, in order to calculate the employment rate.
:::
...
I trained a logistic regression model to try to predict an employee's salary category (>50K or <=50K) based on various personal characteristics (age, sex, marital status, education level, etc.). Let's take a look at the top 6 absolute feature coefficients!
...
As seen in @fig-logistic_importances, the _capital-gain_ feature greatly affects a person's salary. Capital gain is revenue that you recieve after selling a capital asset. This may include investments and real-estate [@investopedia_capital_gain]. Other important features like _occupation_ and _education_ show us that a good education and important job title will also affect a person's salary.

### Question 4

- Which US state has the greatest average wage?
...
The map (@fig-choropleth_hrly_wage) shows that states like New York and California have an average hourly pay of around $37. On the low end, the state of Mississippi has a mean hourly wage of only $22.87.
...
The bubble chart in @fig-bubble shows that as employment rate increases, so does the annual average salary. This may indicate that with a higher salary, an employee is more liekly to stay in the position. The average median salary becomes greater as the employment rate increases. 
