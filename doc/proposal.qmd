...
Both datasets must be prepared before any operations are started, all NA values will either be replaced or removed. Each dataset will be explored and all unnecessary columns will be removed. The datasets will be used separately from each other to answer any research questions. 

# Research Questions and Methodology

1. Is a job more demanding based on the higher level of education and the age of the person? Does the income level of a job determine how hard a person has to work? This is a task for the adult.data file. For the first question, using a scatterplot, the age can be plotted against the weekly hours worked. In turn the points are colored by the level of education. To create a more presentable visual, education level will be grouped into 3 categories (Preschool, Primary, Secondary, Higher). For the second part, using the salary categories (e.g., <=50K or >50K), a boxplot is created to show how income level relates to weekly hours worked across education categories. The visual will be as such, hours worked will be plotted against
Estimated time: ~6 hours

2. Is there a noticable trend between a person's salary and their sex? What about their race? Multiple visuals will be implemented using the adult.data dataset. The total occurence of each sex will be plotted using a pie chart, with the total being divided by the salary categories (<=50k and >50k). As for race, there will be a stacked bar plot showing the different races, each column representing the total number of race occurences and being stacked by the salary categories. 
...
4. Which US state that the greatest average annual wage and hourly wage? Using the state_M2023_dl.xlsx dataset, the data will be filtered to only contain US states. It will then be further cleaned to only contain a occ_title of 'all occupations'. Then, two bar plots will be made to visualize all 50 states with their corresponding mean annual wages and mean hourly wages. Lastly, to visualize the locations of the US states, two choropleth maps will be created to show a gradient of both annual and hourly average wages throughout the US.
Estimated time: ~5 hours
...
* Begin work on Question 4 (2.5 hours)
...
* Finish Question 4 code and interpret (2.5 hours)
...
* Tidy up report and code (4 hours)
* Continue with report (3 hours)

**Week 12 (4/14 - 4/20):**

* write notes for blog post (4.5 hours)
